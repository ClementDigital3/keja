...
import { KENYAN_CITIES, TYPES } from '../data'
import PropertyCard from '../components/PropertyCard'

export default function Listings({ savedIds, toggleSave, setSelectedListing, setPage, initialSearch, initialCity, listings = [], currentUser }) {
  ...
  const filtered = listings
    .filter(l => {
      ...
      const titleMatch = l.title ? l.title.toLowerCase().includes(q) : false
      const locMatch = l.location ? l.location.toLowerCase().includes(q) : false
      const cityMatch = l.city ? l.city.toLowerCase().includes(q) : false
      const addrMatch = l.address ? l.address.toLowerCase().includes(q) : false
      const descMatch = l.desc ? l.desc.toLowerCase().includes(q) : false
      
      const searchMatch = !search || titleMatch || locMatch || cityMatch || addrMatch || descMatch

      // City filter
      const cityFilterMatch = city === "All Cities" || (l.city && l.city.toLowerCase() === city.toLowerCase())

      // Type filter
      const typeFilterMatch = type === "All Types" || l.type === type

      // Price filter
      const priceFilterMatch = l.price <= maxPrice

      // Beds filter
      let bedNum = 0
      if (l.beds) {
        if (typeof l.beds === 'number') bedNum = l.beds
        else if (typeof l.beds === 'string') bedNum = parseInt(l.beds) || 0
      }
      const bedsFilterMatch = bedNum >= minBeds

      // Availability filter
      const availFilterMatch = !availOnly || l.available

      return searchMatch && cityFilterMatch && typeFilterMatch && priceFilterMatch && bedsFilterMatch && availFilterMatch
    })
    .sort((a, b) => {
    ...
    <div style={{ maxWidth: 1140, margin: "0 auto", padding: "36px 24px 80px" }}>
      ...
      {showFilters && (
        <div style={{
          ...
          {/* City */}
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8, color: C.textMuted, marginBottom: 7 }}>City</div>
            <select style={inputStyle} value={city} onChange={e => setCity(e.target.value)}>
              {KENYAN_CITIES.map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
          {/* Type */}
          <div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "80px 24px", color: C.textMuted }}>
          ...
          <div style={{ fontSize: 14 }}>Try broadening your search or adjusting the filters.</div>
        </div>
      ) : (
