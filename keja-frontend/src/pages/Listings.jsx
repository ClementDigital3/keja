...
export default function ListingForm({ setPage, currentUser, onAddListing }) {
  ...
  const cityAreas = form.city ? (CITY_AREAS[form.city] || ['Other']) : []
  ...
  const validate = () => {
    ...
    if (step === 1) {
      if (!form.city)           return 'Select a city or town.'
      if (!form.area)           return 'Select an area within the city.'
      if (!form.address.trim()) return 'Enter a specific street, estate, or building name.'
          ...
          {step === 1 && (
            <div>
              ...
              <div style={{ marginBottom: 16 }}>
                {label('City / Town', true)}
                <select style={inp} value={form.city} onChange={e => {
                  set('city', e.target.value)
                  set('area', '')
                  set('location', e.target.value)
                }}>
                  <option value=''>Select city or town...</option>
                  {Object.keys(CITY_AREAS).filter(c => c !== 'Other').sort().map(c => (
                    <option key={c}>{c}</option>
                  ))}
                  <option value='Other'>Other</option>
                </select>
                <Hint>Choose the main city or town your property is located in</Hint>
              </div>

              {/* Area — only shows after city selected */}
              {form.city && (
                <div style={{ marginBottom: 16 }}>
                  {label(`Area / Estate in ${form.city}`, true)}
                  <select style={inp} value={form.area} onChange={e => {
                    set('area', e.target.value)
                    set('location', e.target.value !== 'Other' ? `${e.target.value}, ${form.city}` : form.city)
                  }}>
                    <option value=''>Select area in {form.city}...</option>
                    {cityAreas.map(a => <option key={a}>{a}</option>)}
                  </select>
                  <Hint>Pick the specific neighbourhood, estate, or area within the city</Hint>
                </div>
              )}
              ...
              {form.area === 'Other' && (
                <div style={{ marginBottom: 16 }}>
                  {label('Specify area name', true)}
                  <input style={inp} placeholder={`e.g. Maili Tatu, ${form.city}`}
                    value={form.address} onChange={e => {
                      set('address', e.target.value)
                      set('location', `${e.target.value}, ${form.city}`)
                    }} />
                </div>
              )}
              ...
              {form.city && form.area && (
                <div style={{ marginBottom: 16 }}>
                  {label('Street / Building / Estate name', true)}
                  <input style={inp} placeholder='e.g. Elgon View Drive, Sunrise Apartments, Plot 45'
                    value={form.area !== 'Other' ? form.address : ''}
                    onChange={e => set('address', e.target.value)} />
                  <Hint>The most specific location detail — helps tenants find you</Hint>
                </div>
              )}
              ...
              {form.city && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <input style={inp} placeholder="e.g. Near Nakumatt, Opposite St. Mary's Hospital"
                    value={form.landmark} onChange={e => set('landmark', e.target.value)} />
                  <Hint>Helps tenants locate the property easily</Hint>
                </div>
              )}
